...
//--------------------------------------
// ネイティブインスタンスのポインタを取得する部分を独自に书き记したい场合のサンプル

NCB_GET_INSTANCE_HOOK(PadAttachTest2)
{
	// スコープ内ではあらかじめ typedef PadAttachTest2 ClassT; と定义されている
	...
	NCB_INSTANCE_GETTER(objthis) { // objthis を iTJSDispatch2* 型の引数とする
		...
		// ポインタ取得
		ClassT* obj = GetNativeInstance(objthis); //< ネイティブインスタンス取得组み込み関数
		if (!obj) {
			// ない场合は生成する
			obj = new ClassT();

			// objthis に obj をネイティブインスタンスとして登録する（忘れると毎回 new されますよー）
			SetNativeInstance(objthis, obj); //< ネイティブインスタンス设定组み込み関数
		}

		// インスタンス侧にobjthisを持たせてほげほげしたい场合はこんな感じで
		if (obj) obj->SetObjthis(objthis);

		// デストラクタで使用したい场合はプライベート変数に保存
		_objthis = objthis;
		_obj = obj; 
...
}; // 実体は class 定义なので ; を忘れないでね


/// 通常アタッチ（インスタンスはメソッドが初めて呼ばれる时にnewされる）
NCB_ATTACH_CLASS(PadAttachTest1, Pad) {
	Method("Test1", &Class::Test1); //NCB_METHOD(Test1);
}

// フックつきアタッチ（あらかじめ NCB_GET_INSTANCE_HOOK が定义されていること：ない场合はコンパイルエラー）
