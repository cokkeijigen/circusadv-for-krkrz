...
struct ncbInstanceAdaptor : public tTJSNativeInstance {
	...
	// 上记マクロを使用せずに独自実装しているここでは使用しない(⇒ncbNativeClassConstructor)

	/// オブジェクトが无効化されるときに呼ばれる
...
public:
	...
	/// アダプタを生成しつつNativeClassインスタンスを设定
	...
	static iTJSDispatch2* CreateAdaptor(NativeClassT *inst, bool sticky = false, bool err = false) {
		...
		// 引数が1つでかつvoidであれば実インスタンスをnewしない动作になる
...
#define NCB_INNER_CONVERSION_SPECIALIZATION \
	...
	/// 修饰子取り外し(ポインタ，参照，const等を外した素の型が typedef される)
...
//--------------------------------------
/// 返り値の型を正确に取りたい场合，コンバータはこれを継承する
struct ncbStrictResultConvertor {};

//--------------------------------------
// TypeConvertor関连の各种マクロ
...
struct ncbNarrowCharConvertor {
	/// 一时的にバッファを确保してそこに NarrowStr として书き込む
	struct ToNChar {
		/// Constructor (メソッドが呼ばれる前)
		ToNChar() : _nstr(0) {}
		/// Destructor (メソッドが呼ばれた后)
		~ToNChar() {
			if (_nstr) {
...
struct ncbNativeObjectBoxing {
	...
	struct Boxing : public ncbStrictResultConvertor {
		...
		inline void operator ()(VarT &dst, SRC src, ncbTypedefs::Tag<SRC> const&) const {
			...
			ClassT *p = box<TargetT>::Get(src);						//< コピー/参照/ポインタ场合分け
	...
	struct Unboxing {
		...
		inline void operator ()(DST &dst, VarT const &src) const {
			...
			dst = ConvT::ToTarget<TargetT>::Get(p);					//< 必要とされる型に変换して返す
		}
	};
};
...
/// 特殊化用のマクロ (自动変换でなく直で指定する场合はこのマクロを使用する)
...
/// 返り値なしの场合のダミーの TOVARIANT を登録
NCB_SET_TOVARIANT_CONVERTOR(void, struct {});

// iTJSDispatch2*を引き数・返り値にする场合
...
#define FCALL_SET_EXT(num) params[ num - 1 ] = &param ## num;
	// FuncCallを任意个数のtTJSVariantを引数で受けるようなメソッドを展开
#undef  FOREACH
#define FOREACH \
...
//		case nitClass:    _name = TJS_W("Class");    break; // クラスになることはありえない
	...
	{
		// 自分自身(membername==0)で比较クラス名が_nameならTRUE，それ以外は丸投げ
...
private:
	...
	struct paramsFunctor {
		...
		inline T operator ()(CallerT::tNumTag<N> const& /*index*/, CallerT::tTypeTag<T> const& /*type*/) {
			...
			ParamT ret;
			// N番目の ncbToValueConvertor を取り出して変换
		...
		/// NativeClassMethod の返り値をresultへ格纳
		...
		inline bool SetResult(ResultT r, DefsT::Tag<ResultT> const&, DefsT::BoolTag<false> const&) {
			if (_result) _rconv(*_result, r); // ncbToVariantConvertor で返り値に変换
			return true;
		}
	...
	// 先头にインスタンスポインタを渡すPROXY METHODのファンクタ
	...
	//--------------------------------------
private:
	/// ネイティブインスタンスを取得するためのテンプレート（特殊化で上书きできるようにするため
	template <class T>
...
protected:
	...
	struct doInvoke : public doInvokeBase {
		...
		//enum { ArgsCount = SelectorT::ArgsCount };           // 引数の个数(proxy なら-1された値)
...
public:
	struct InvokeType {
		...
		// Proxy : Normal から FunctorT と ArgsCount を置き换える
		...
		// ProxyBridge : Bridge から FunctorT と ArgsCount を置き换える
...
// 自前で実装する(TJSCreateNativeClassMethodではstaticな関数しか呼べないのでメソッドへのポインタの保持が困难なため)
...
struct ncbNativeClassConstructor : public ncbNativeClassMethodBase {
		...
		tTJSVariant *result, tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *objthis)
	{
		...
		// 引き数がひとつでかつvoidの场合はインスタンスを设定しない
...
struct ncbRegistNativeClass : public ncbRegistNativeClassBase {
	...
	void RegistEnd() {
		// コンストラクタがない场合はエラーを返すコンストラクタを追加する
		...
		// 2 _classobj を tTJSVariant 型に変换
		...
		global->PropSet(
			TJS_MEMBERENSURE, // メンバがなかった场合には作成するようにするフラグ
			_className, // メンバ名
		...
		// が Release されず、次に使う TVPPluginGlobalRefCount が正确にならない。
		//val.Clear();
		// …のだがローカルスコープで自动的に消去されるので必要ない

		NCB_LOG_2(TJS_W("EndRegistClass: "), _className);
	}

	/// プラグイン开放时にクラスオブジェクトをリリースする
	void UnregistEnd() {
		iTJSDispatch2 *global = TVPGetScriptDispatch();
		if (global) {
...
////////////////////////////////////////
/// TJSクラス自动レジストクラス（スレッドアンセーフなので必要なら适当に修正のこと）
